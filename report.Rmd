...
We notice no outliers when it comes to the percentage of residents with an education level less than High-School. We also 
...
We can also visualize Foursquare data as a box plot. In this plot, the number of venues (normalized to the Foursquare
...
The next figure also shows the relationship between data features extracted from NYC neighborhood stats data set: It seems these 
...
To cluster neighborhoods and cities based on their livability score, we use Python's sci-kit learn library to 
define a function which takes as input the data frame, a list of target variables, the number of clusters
...
Which suggests that NYC can be divided into two clusters, one where living is easy and the other where living is harder
...
The limitation on Foursquare developer accounts greatly affected the quality of data we could fetch from their API,
and the data was biased towards restaurants and coffee shops, but, combining it with another data set was enough to hide 
the biased effect.
...
Crime, Education and Employment; which were roughly estimated using 16 other features.
